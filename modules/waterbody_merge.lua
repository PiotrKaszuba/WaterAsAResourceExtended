...
function waterbody_merge.mergeGridsData(gridsData, other_gridsData, sourceWaterBodyId, targetWaterBodyId)
	utils.LazyTables.on_merge(gridsData.lazyWaterGridWithData, other_gridsData.waterGridWithData,
	...
	utils.LazyTables.on_merge(gridsData.lazyDriedTilesGridWithData, other_gridsData.driedTilesGridWithData,
...
function waterbody_merge.mergeWaterBodyStateData(waterBodyStateData, other_waterBodyStateData, waterBodyId)
	...
	waterBodyStateData.TempAvailableWater = waterBodyStateData.TempAvailableWater +
	other_waterBodyStateData.TempAvailableWater
	waterBodyStateData.TempUsedWater = waterBodyStateData.TempUsedWater + other_waterBodyStateData.TempUsedWater

	waterBodyStateData.WaterUsedPenalty = waterBodyStateData.WaterUsedPenalty + other_waterBodyStateData
	.WaterUsedPenalty
	waterBodyStateData.WaterUsedPenaltyRestored = waterBodyStateData.WaterUsedPenaltyRestored +
	other_waterBodyStateData.WaterUsedPenaltyRestored
	...
	other_waterBodyStateData.OrphanedSecondsCount
...
function waterbody_merge.signalWaterBodyMergedToPlayer(waterBody, force, other_waterBody)
	return string.format("%s merged %s. Merged water body has %sL of water with regen %sL and total area of %s tiles.",
		waterbodies.getFullNameForWaterBody(waterBody), waterbodies.getFullNameForWaterBody(other_waterBody),
		utils.comma_value(waterBody.waterAreaData.AmountWtr), waterBody.waterAreaData.RegenAmount,
		waterBody.waterAreaData.TotalArea)
end
...
function waterbody_merge.mergeWaterBody(waterBody1, waterBody2)
	...
	waterbody_merge.mergeGridsData(waterBody1.gridsData, waterBody2.gridsData, waterBody1.waterBodyId,
		waterBody2.waterBodyId)
	...
	waterbody_merge.mergeWaterBodyStateData(waterBody1.waterBodyStateData, waterBody2.waterBodyStateData,
		waterBody1.waterBodyId)

	waterbody_merge.mergeShapeData(waterBody1.waterBodyShapeData, waterBody2.waterBodyShapeData)

	waterbody_merge.mergeWaterBodyTileCountData(waterBody1.waterBodyTileCountPercentagePenalty,
		waterBody2.waterBodyTileCountPercentagePenalty)
