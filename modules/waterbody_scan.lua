...
function waterbody_scan.getAdjacentWaterAndLandTiles(position, surface, water_body_id_or_ids,
													 expand_waterbody_owned_land_tiles_depth)
...
function waterbody_scan.recalculateEdgesAroundPosition(waterBody, position, surface, updateBudget)
	local adjacent_waterbody_tiles, adjacent_land_tiles, _ = waterbody_scan.getAdjacentWaterAndLandTiles(position,
		surface, waterBody.waterBodyId)
...
function waterbody_scan.EdgePattern(
	...
	for _, offset in pairs(utils.AdjacentOffsets) do
		...
		local already_searched = tileWaterBodyId == waterBodyId or
		utils.LazyTables.get(gridKey, edgeGrid, lazyEdgeGrid) ~= nil
		if (not already_searched) then
			if not tile.valid then
				tileInvalidOrOutOfMap(surface, position, true, searchQueue, search_queue_enqueue,
					"waterbody_scan.EdgePattern")
				goto continue
			end
			...
			if tile_name == out_of_map_tile_name then
				tileInvalidOrOutOfMap(surface, position, false, searchQueue, search_queue_enqueue,
					"waterbody_scan.EdgePattern")
				goto continue
			end
				...
				elseif tileWaterBodyId ~= -1 then
					...
					if tileWaterBody then
						if tileWaterBody.valid then
							utils.profile_hits("waterbody_scan.EdgePattern",
								"got non water tile of ANOTHER waterbody! IT IS VALID!")
							...
							if not explanation_cond then
								utils.profile_hits("waterbody_scan.EdgePattern",
									"got non water tile of ANOTHER waterbody! IT IS INVALID! AND NOT A PARENT OF THE WATERBODY!")
								game.print(
								"Testing: EdgePattern got non water tile of ANOTHER waterbody! IT IS INVALID! AND NOT A PARENT OF THE WATERBODY!")
							end
						end
					else
						utils.profile_hits("waterbody_scan.EdgePattern",
							"got non water tile of ANOTHER waterbody! NO WATERBODY REFERENCE!")
						game.print(
						"Testing: EdgePattern got non water tile of ANOTHER waterbody! NO WATERBODY REFERENCE!")
					end
					-- fix stale ref
					addNewWaterTile(gridKey, surfaceName, -1)
				end
...
function waterbody_scan.processWaterTile(
	...
	if is_dry_tile then
		original_tile_name = utils.LazyTables.get(gridKey, OrphanedDryTilesOriginalName[surfaceName],
			lazyOrphanedDryTilesOriginalName[surfaceName])
	...
	if waterBodyTileType ~= nil then
		...
		if new_water_body_id ~= waterBodyId then
			...
			if not hot_utils.isTileInGrid(waterGridWithData, lazyWaterGridWithData, driedTilesGridWithData, lazyDriedTilesGridWithData, gridKey) then
				...
				waterBodyTileCountPercentagePenalty[waterBodyTileType] = waterBodyTileCountPercentagePenalty
				[waterBodyTileType] + tile_percentage_water_used
				...
				if is_dry_tile then
					...
					utils.LazyTables.remove(gridKey, OrphanedDryTilesOriginalName[surfaceName],
						lazyOrphanedDryTilesOriginalName[surfaceName])
		...
		EdgePattern(position, surface, surfaceName, water_body, false,
			...
			GetTile, GridKey, getWaterTile, IsWaterOrDryTile, search_queue_enqueue, addNewWaterTile,
			tileInvalidOrOutOfMap, out_of_map_tile_name
...
function waterbody_scan.tileInvalidOrOutOfMap(surface, search_position, invalid, search_queue, search_queue_enqueue,
											  caller_name)
...
function waterbody_scan.ScanWaterArea(water_body, search_amount, updateBudget)
	...
	-- local variables
	local search_position, tile, gridKey, tile_waterBodyId, already_searched, added_tile, waterbody_changed, total_area_increase, dried_tiles_increase =
	nil, nil, nil, nil, false, false, false, 0, 0
	...
	local batchMinX, batchMaxX, batchMinY, batchMaxY, batchSumX, batchSumY, batchTileCount = huge_val, -huge_val,
		huge_val, -huge_val, 0, 0, 0
	...
	while not is_empty(search_queue) and search_amount > 0 do
		...
		if not tile.valid then
			local dropped_tile = tileInvalidOrOutOfMap(surface, search_position, true, search_queue, search_queue_enqueue,
				"waterbody_scan.ScanWaterArea")
		...
		if tile_name == out_of_map_tile_name then
			local dropped_tile = tileInvalidOrOutOfMap(surface, search_position, false, search_queue,
				search_queue_enqueue, "waterbody_scan.ScanWaterArea")
			if dropped_tile then goto continue end
			break
		end
		...
		if not already_searched then
			if totalArea <= max_water_body_size then
				water_body, added_tile, total_area_increase, dried_tiles_increase, waterbody_changed = processWaterTile(
					...
					waterbody_id, gridsData, waterGridWithData, lazyWaterGridWithData, driedTilesGridWithData,
					lazyDriedTilesGridWithData,
					...
					ValidWaterBodies, OrphanedDryTilesOriginalName, lazyOrphanedDryTilesOriginalName,
					WaterTileToWaterBodyTileType,
					getWaterTilePercentageWaterUsed,
				...
				if waterbody_changed then
					...
					waterBodyTileCountData = water_body.waterBodyTileCountData
					waterBodyTileCountPercentagePenalty = water_body.waterBodyTileCountPercentagePenalty
					search_queue_enqueue, search_queue_dequeue = waterbodies.getSearchQueueEnqueueAndDequeue(
					search_queue)
				end
...
function waterbody_scan.signalFinishedScanningToPlayer(water_body)
	...
	return string.format("%s %s, with %sL of water with regen %sL and total area of %s tiles.",
		waterbodies.getFullNameForWaterBody(water_body), msg_type, utils.comma_value(water_body.waterAreaData.AmountWtr),
		water_body.waterAreaData.RegenAmount, water_body.waterAreaData.TotalArea)
end
...
function waterbody_scan.scanningLoopPeriodic(water_body)
	water_body.waterBodyStateData.ScanLoopCount = water_body.waterBodyStateData.ScanLoopCount +
	utils.normalize_update_values_per_second(1, false, storage.PeriodicTicksPerScanningUpdate)
...
function waterbody_scan.prepareDistributedBudgetUpdateForValidWaterBodies(total_work_amount, updateBudget, condition_func,
																		  initial_work_weight)
...
function waterbody_scan.scanningUpdateAll(updateBudget)
	...
	.prepareDistributedBudgetUpdateForValidWaterBodies(waterbody_scan.getAdditionalScanAmount(), updateBudget,
		waterbody_scan.prepareUpdateConditionFunc)
